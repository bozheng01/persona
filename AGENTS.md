...
- Always keep the local git repo in sync with `origin/main` (github.com/bozheng01/persona.git). **Auto-deploy**: once a change is done and verified, commit and push to `origin/main` without asking (the user opted in to automatic deployment). Never commit secrets.